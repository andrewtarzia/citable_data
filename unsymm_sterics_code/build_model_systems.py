...
def host_optimisation_sequence(mol, name, charge, calc_dir):
    xtbopt_output = os.path.join(calc_dir, f"{name}_xtb.mol")
...
def main():
    ...
    structure_dir = pathlib.Path("./model_systems")
    ...
    for i in ligands:
        ...
        for c_a in c_orientations:
            ...
            if "l1" in i:
                ...
                energies[i][f"{c_a}_cl"] = get_xtb_energies(
                    mol=host_guest,
                    name=host_name,
                    charge=3,
                    calc_dir=calculation_dir,
                ...
                for atom in host_guest.get_atoms():
                    x, y, z = next(
                        host_guest.get_atomic_positions(atom.get_id())
                    )
                ...
                energies[i][f"{c_a}_cl_bf4"] = get_xtb_energies(
                    mol=host_guest,
                    name=host_name,
                    charge=1,
                    calc_dir=calculation_dir,
